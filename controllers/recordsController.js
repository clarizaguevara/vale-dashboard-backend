...
exports.getRecord = async (request, response, next) => {
    try {
        ...
        if (param.searchBy.toLowerCase() === "name" && param.searchKey) {
            sqlQuery += `
            WHERE CONCAT(p.first_name, ' ', p.middle_name, ' ', p.last_name) LIKE '%${param.searchKey}%' `
        }

        if (param.searchBy.toLowerCase() === "date_of_access" && param.searchKey) {
            sqlQuery += `
            WHERE m.date_of_access = '${param.searchKey}' `
        }

        if (param.searchBy.toLowerCase() === "location" && param.searchKey) {
            sqlQuery += `
            WHERE l.location_name LIKE '%${param.searchKey}%' `
        }

        sqlQuery += `
            ORDER BY ${param.orderBy || 'date_of_access'} ${param.order || 'DESC'} `
        sqlQuery += `
            LIMIT 20 `
